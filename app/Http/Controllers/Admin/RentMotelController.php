...
class RentMotelController extends Controller
{
	public function index(Request $request) {
		return view('Admin.RentMotel.index');
	}
	public function Datatable(Request $request) {
		$property = Property::with('user')->where('property_type', 0)->orderBy('property_id', 'desc')->get();
		$array = [];
		...
		->addColumn('action', function ($action) {
			...
			return '
			<a href="'.route('ListRentMotelView', $action['property_id']).'" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-eye-open"></i> Xem</a>
	...
	public function View(Request $request, $id) {
		...
		return view('Admin.RentMotel.view', compact('property'));
	}
